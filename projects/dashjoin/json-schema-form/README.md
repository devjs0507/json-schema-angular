...
@Component({
  selector: 'app-root',
  template: `
    <ca-json-schema-form [(value)]="value" [schema]="schema" [label]="schema.title"></ca-json-schema-form>
    <pre>{{print()}}<pre>
  `
})
...
```
<ca-json-schema-form [(value)]="value" [schema]="schema" [label]="schema.title" (errorChange)="error=$event">
</ca-json-schema-form>
...
```
<ca-json-schema-form [schema]="{$ref:'https://raw.githubusercontent.com/riskine/ontology/master/schemas/core/profession.json'}">
```

The URL can also be relative to the form's URL:

```
<ca-json-schema-form [schema]="{$ref:'schema.json'}">
```
